...
//				        player.closeInventory();
	    ...
	    .onComplete((player, text) -> {           //called when the inventory output slot is clicked
	    	...
	    	if (op.getPlayer() == null) {
			    ...
			    if (!op.hasPlayedBefore()) {
			    	player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(player, ConfigManager.uIAnvilNeverPlayed), 1));
			...
			if (ReferralPro.Instance.db.PlayerReferrald(receiverString)) {
				player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(player, ConfigManager.uIAnvilAlreadyRefed), 1));
	            return AnvilGUI.Response.text("");
			}
			...
			if (senderString.compareTo(receiverString) == 0) {
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(player, ConfigManager.uIAnvilCantRefSelf), 1));
	            return AnvilGUI.Response.text("");
			}
			
			// Check if the request already exists
			if (ReferralPro.Instance.db.RequestExists(senderString, receiverString)) {			
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(player, ConfigManager.uIAnvilAlreadyInQ), 1));
	            return AnvilGUI.Response.text("");
			}
			...
			if (ReferralPro.Instance.db.PlayerReferrald(p.getUniqueId().toString())) {
				...
				if (!ReferralPro.Instance.getConfig().getBoolean("canReferEachOther") && refedName.equalsIgnoreCase(text)) {			
			        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.tryRefEachOther), 1));
		            return AnvilGUI.Response.text("");
				}
			}
			...
			if (!ReferralPro.Instance.getConfig().getBoolean("canReferOlderPlayer") && p.getFirstPlayed() > op.getFirstPlayed()) {			
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.tryRefOlderPlayer), 1));
	            return AnvilGUI.Response.text("");
			}
	        ...
	        if (minTime != -1 && ((op.getLastPlayed() - op.getFirstPlayed()) / 60000) < minTime) {
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.notEnoughPlayTime), 1));
	            return AnvilGUI.Response.text("");
			}
	        ...
	        // Check if server uses time limit if so did the player play enough
 	        ...
 	        if (maxTime != -1 && ((op.getLastPlayed() - op.getFirstPlayed()) / 60000) > maxTime) {
 		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.tooMuchPlayTime), 1));
	            return AnvilGUI.Response.text("");
 			}
 	        ...
 	        if (maxRequests != -1 && ReferralPro.Instance.db.TotalRequests(senderString) >= maxRequests) {
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(player, ConfigManager.maxRequestSend), 1));
	            return AnvilGUI.Response.text("");
			}
 	        ...
 	        if (!ReferralPro.Instance.getConfig().getBoolean("allowSameIPRefer") && p.getAddress().getHostName().equals(ReferralPro.Instance.db.GetPlayerIP(receiverString))) {
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.sameIPRefer), 1));
	            return AnvilGUI.Response.text("");
			}
			...
			if (ReferralPro.Instance.db.BlockExists(senderString, receiverString)) {			
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.uIAnvilBlocked), 1));
	            return AnvilGUI.Response.text("");
			}
		    ...
		    if (playerReceiver == null) {
		        // Not Online
		        player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.uIAnvilRequestQed), 0));
	            return AnvilGUI.Response.text("");
		    }
		    ...
		    player.openInventory(UIAnvilResponse.GUI(Utils.FormatString(op, ConfigManager.uIAnvilInvited), 0));
            return AnvilGUI.Response.text("");			
	    })
	    ...
	    .title(Utils.FormatString(p, ConfigManager.uIAnvilTitle))   											//set the title of the GUI (only works in 1.14+)
